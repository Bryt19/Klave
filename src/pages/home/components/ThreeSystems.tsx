...
export default function ThreeSystems() {
  return (
    <section id="three-systems" className="relative py-24 md:py-32 bg-white dark:bg-slate-900 transition-colors duration-200">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
          
          {/* Left Column - Sticky Headline & Context */}
          <div className="lg:col-span-5 lg:sticky lg:top-28">              <Reveal>
              <h2 className="text-2xl sm:text-3xl lg:text-5xl font-bold tracking-tight text-slate-900 dark:text-white leading-[1.15] sm:leading-[1.12] mb-5 sm:mb-6">
                Three systems, <br className="hidden sm:block" />
                one confident <br className="hidden sm:block" />
                <span className="text-slate-900 dark:text-white">workflow.</span>
              </h2>

              <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 font-normal leading-relaxed max-w-md mb-6 sm:mb-8">
              ...
              <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-700/50 hidden sm:block">
                {threeSystems.map((sys) => (
                  <div key={sys.number} className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-300 font-medium">
                    <span className="w-5 h-5 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 font-mono text-[11px] flex items-center justify-center font-bold">
                      {sys.number}
                    </span>
                    <span>{sys.title}</span>
                  </div>
          ...
          <div className="lg:col-span-7 space-y-6 sm:space-y-10">
              ...
              <div className="rounded-3xl p-5 sm:p-8 organic-card-mesh border border-emerald-100 dark:border-emerald-800/40/80 shadow-sm transition-all duration-300 hover:shadow-md">
                ...
                <div className="bg-white/90 backdrop-blur-md rounded-2xl p-3 sm:p-4 border border-slate-200 dark:border-slate-700/80 shadow-xs space-y-2 sm:space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-1.5 sm:gap-2 text-xs">
                    <div className="p-1.5 sm:p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/50">
                      <div className="text-[10px] text-slate-400">Patient</div>
                      <div className="font-semibold text-slate-800 dark:text-slate-100 truncate">Eleanor Vance</div>
                    </div>
                    <div className="p-1.5 sm:p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/50">
                      <div className="text-[10px] text-slate-400">Rx</div>
                      <div className="font-semibold text-slate-800 dark:text-slate-100 truncate">Lisinopril 20mg</div>
                    </div>
                    <div className="p-1.5 sm:p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/50">
                      <div className="text-[10px] text-slate-400">Prescriber</div>
                      <div className="font-semibold text-slate-800 dark:text-slate-100 truncate">Dr. K. Appiah</div>
                    </div>
                    <div className="p-1.5 sm:p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/50">
                      <div className="text-[10px] text-slate-400">Qty</div>
                      <div className="font-semibold text-slate-800 dark:text-slate-100 truncate">90 Tablets</div>
                    </div>
                  </div>
            ...
            <Reveal delay={0.2}>
              <div className="rounded-3xl p-5 sm:p-8 organic-emerald-card border border-emerald-200/80 shadow-sm transition-all duration-300 hover:shadow-md">
                ...
                <div className="bg-white/90 backdrop-blur-md rounded-2xl p-2.5 sm:p-4 border border-slate-200 dark:border-slate-700/80 shadow-xs space-y-1.5 sm:space-y-2.5">
                  <div className="flex items-center justify-between text-xs pb-2 border-b border-slate-100 dark:border-slate-700/50">
                    <span className="font-semibold text-slate-800 dark:text-slate-100">Batch Allocation</span>
                  ...
                  {[
                    ...
                    <div
                      ...
                      className={`p-2 sm:p-2.5 rounded-xl text-[11px] sm:text-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-0 ${
                        row.highlight
                          ? "bg-emerald-50 dark:bg-emerald-900/30/90 border border-emerald-200 text-emerald-950 font-medium"
                          : "bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/50 text-slate-600 dark:text-slate-300"
                      }`}
                    >
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 pl-5 sm:pl-0">
                        ...
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${row.highlight ? "bg-emerald-200 text-emerald-900" : "bg-slate-200 dark:bg-slate-700 text-slate-700"}`}>
                          {row.status}
                        </span>
                      </div>
                    </div>
            ...
            <Reveal delay={0.3}>
              <div className="rounded-3xl p-5 sm:p-8 organic-card-mesh border border-emerald-100 dark:border-emerald-800/40/80 shadow-sm transition-all duration-300 hover:shadow-md">
                ...
                <div className="bg-white/90 backdrop-blur-md rounded-2xl p-2.5 sm:p-4 border border-slate-200 dark:border-slate-700/80 shadow-xs space-y-2 sm:space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2 text-[11px]">
                    <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700/50 flex items-center gap-1.5 text-slate-700">
