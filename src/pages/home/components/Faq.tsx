          ...
          <Reveal delay={0.1}>
            <div className="space-y-3">
              {faqs.map((faq, i) => {
                ...
                return (
                  <div key={faq.question} className={`rounded-2xl border transition-all duration-200 overflow-hidden ${open ? "border-emerald-200 bg-emerald-50/30 shadow-sm" : "border-slate-200/70 bg-white hover:border-slate-300"}`}>
                    <motion.button
                      type="button"
                    ...
                    >
                      <span className={`text-sm font-semibold transition-colors ${open ? "text-emerald-800" : "text-slate-900"}`}>{faq.question}</span>
