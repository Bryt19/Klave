...
function StockBar({ stock, maxStock, status }: { stock: number; maxStock: number; status: string }) {
  ...
  return (
    <div className="w-full h-1.5 rounded-full bg-slate-100 dark:bg-slate-800/50 overflow-hidden">
      <motion.div
        initial={{ width: 0 }}
  ...
  return (
    <section ref={containerRef} id="features" className="relative mt-12 pt-20 sm:pt-28 lg:pt-36 pb-14 sm:pb-20 lg:pb-28 bg-slate-50/50 dark:bg-slate-900 overflow-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Reveal delay={0.15}>
          <div className="rounded-2xl sm:rounded-3xl overflow-hidden bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 dashboard-shadow">
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-700/50">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center">
                  <i className="ri-archive-line text-emerald-600 text-sm" />
                </div>
                <div>
                  <div className="text-sm font-bold text-slate-900 dark:text-white">Inventory Overview</div>
                  <div className="text-[11px] text-slate-400">{inventoryMedicines.length} medicines tracked</div>
                </div>
              </div>
              <div className="hidden sm:flex items-center gap-2">
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 text-[11px] text-slate-500 dark:text-slate-400">
              ...
              {inventoryMedicines.map((med, i) => (
                ...
                >
                  <div className="sm:col-span-4 flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="text-xs font-semibold text-slate-800 dark:text-slate-100">{med.name}</div>
                      <div className="text-[10px] text-slate-400 sm:hidden">{med.category}</div>
                    </div>
                  </div>
                  <div className="sm:col-span-2 hidden sm:flex items-center">
                    <span className="text-[11px] font-mono text-slate-500 dark:text-slate-400">{med.batch}</span>
                  </div>
                  <div className="sm:col-span-3 flex flex-col justify-center gap-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-semibold text-slate-700 dark:text-slate-300">{med.stock} / {med.maxStock}</span>
                  ...
                  <div className="sm:col-span-2 hidden sm:flex items-center">
                    <span className={`text-[11px] font-medium ${
                      med.status === "expiring" ? "text-red-600" : "text-slate-500 dark:text-slate-400"
                    }`}>{med.expiry}</span>
                  </div>
                  <div className="sm:col-span-1 flex items-center">
                    <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                      med.status === "out" ? "bg-red-50 dark:bg-red-900/30 text-red-600 border border-red-100 dark:border-red-800/40" :
                      med.status === "low" ? "bg-amber-50 dark:bg-amber-900/30 text-amber-600 border border-amber-100 dark:border-amber-800/40" :
                      med.status === "expiring" ? "bg-red-50 dark:bg-red-900/30 text-red-500 border border-red-100 dark:border-red-800/40" :
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
          {[
            { icon: "ri-alarm-warning-line", title: "Expiry Alerts", desc: "Automatic warnings before medicines expire", bg: "bg-red-50 dark:bg-red-900/30", ic: "text-red-500" },
            { icon: "ri-alarm-warning-line", title: "Low Stock Alerts", desc: "Instant notifications when inventory hits minimum levels", bg: "bg-amber-50 dark:bg-amber-900/30", ic: "text-amber-500" },
            { icon: "ri-bar-chart-grouped-line", title: "Stock Reports", desc: "Real-time analytics across all branches", bg: "bg-blue-50 dark:bg-blue-900/30", ic: "text-blue-500" },
          ].map((item) => (
            <Reveal key={item.title} delay={0.3}>
              <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700/50 flex items-start gap-3 card-hover-glow hover:shadow-md transition-shadow duration-300">
        ...
        <Reveal delay={0.4}>
          <div className="mt-8 sm:mt-10">
            ...
            <div className="text-center mb-10 mt-16">
              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-3">How Klavora Inventory <span className="text-emerald-600">Works</span></h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 max-w-lg mx-auto">A four-step workflow that replaces spreadsheets, paper logs, and manual tracking with intelligent batch-level automation.</p>
            </div>
              ...
              {[
                ...
                <div key={item.step} className={`group p-5 rounded-2xl bg-white dark:bg-slate-800 border ${item.accent} card-hover-glow cursor-default`}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-[10px] font-bold text-slate-300 group-hover:text-slate-500 dark:text-slate-400 transition-colors duration-300">STEP {item.step}</span>
                  </div>
